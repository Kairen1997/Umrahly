defmodule Umrahly.Repo.Migrations.FixBookingsSchemaGracefully do
  ...
  def change do
    # First, guard all logic on the existence of package_schedules to avoid undefined_table errors
    execute """
    DO $$
    DECLARE
    ...
    BEGIN
      -- Only proceed if package_schedules table exists
      IF to_regclass('public.package_schedules') IS NOT NULL THEN
    ...
    # Add package_schedule_id column if it doesn't exist and package_schedules exists
    execute """
    DO $$
    BEGIN
      IF to_regclass('public.package_schedules') IS NOT NULL THEN
